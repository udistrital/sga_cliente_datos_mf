...
@Component({
  selector: 'sga-datos-mf-dynamic-form',
  standalone: true,
  imports: [
    MatFormFieldModule,
    MatDatepickerModule,
...
export class DynamicFormComponent {
  ...
  constructor(
    ...
    this.searchTerm$
      ...
      .subscribe((response: any) => {
        ...
        const fieldAutocomplete = this.normalform.campos.filter(
          (field: any) => field.nombre === response.options.field.nombre
        );
        fieldAutocomplete[0].opciones = opciones;
        if (opciones.length == 1 && Object.keys(opciones[0]).length == 0) {
          let canEmit = fieldAutocomplete[0].entrelazado
            ? fieldAutocomplete[0].entrelazado
            : false;
          if (canEmit) {
            this.interlaced.emit({
              ...fieldAutocomplete[0],
              noOpciones: true,
              valorBuscado: response.keyToFilter,
            });
          }
        }
      });
  ...
  setNewValue({ element, field }: { element: any; field: any }) {
  ...
  searchEntries(
    ...
    const queryOptions$ = this.requestService.get(
      path,
      query.replace(keyToFilter, text)
    );
                  ...
                  switch (element.etiqueta) {
                    case 'selectmultiple':
                      ...
                      if (this.modeloData[i].length > 0) {
                        this.modeloData[i].forEach((e1: any) =>
                      ...
                      if (element.hasOwnProperty('opciones')) {
                        if (element.opciones != undefined) {
                          element.opciones.forEach((e1: any) => {
                            if (
                              this.modeloData[i].Id !== null ||
                              this.modeloData[i].Id !== undefined
                            ) {
  ...
  download(
    ...
    if (noPreview) {
      ...
      prev ? (prev.document.title = title) : null;
    }
  }
  ...
  validlog1(event: any) {
    const camposLog1 = this.normalform.campos.filter(
  ...
  validCampo(c: any, emit = true): boolean {
    ...
    if (
      (c.requerido &&
        ((c.valor === '' && c.etiqueta !== 'file') ||
          c.valor === null ||
          c.valor === undefined ||
          (JSON.stringify(c.valor) === '{}' && c.etiqueta !== 'file') ||
          JSON.stringify(c.valor) === '[]')) ||
      (c.etiqueta === 'file' &&
        c.valor.name === undefined &&
        c.etiqueta === 'file' &&
        (c.urlTemp === undefined || c.urlTemp === '')) ||
      (c.etiqueta === 'file' &&
        c.valor.name === null &&
        c.etiqueta === 'file' &&
        (c.urlTemp === null || c.urlTemp === ''))
    ) {
    ...
    if (
      (c.etiqueta === 'input' || c.etiqueta === 'inputConfirmacion') &&
      c.tipo === 'number'
    ) {
    ...
    if (
      ...
      if (c.valor.size > c.tamanoMaximo * 1024000) {
        c.clase = 'form-control form-control-danger';
        c.alerta =
          'El tamaño del archivo es superior a : ' + c.tamanoMaximo + 'MB. ';
        return false;
      }
      if (c.valor.type) {
    ...
    if (c.etiqueta === 'textarea') {
      ...
      if (c.cantidadCaracteres) {
        if (c.valor.length > c.cantidadCaracteres) {
          c.clase = 'form-control form-control-danger';
          c.alerta =
            'El texto supera el máximo de caracteres permitido (máximo: ' +
            c.cantidadCaracteres +
            ', actualmente: ' +
            c.valor.length +
            ')';
          return false;
  ...
  clearForm() {
    this.normalform.campos.forEach((d: any) => {
      d.valor = null;
      if (d.etiqueta === 'file') {
        const nativeElement = this.DocumentoInputVariable
          ? this.DocumentoInputVariable.nativeElement
            ? this.DocumentoInputVariable.nativeElement
            : null
          : null;
  ...
  validForm() {
    ...
    for (const key in this.modeloData) {
      // Agrega parametros faltantes del modelo
      if (
        this.data.data[this.normalform.modelo] !== undefined &&
        !this.data.data[this.normalform.modelo].hasOwnProperty(key)
      ) {
    ...
    if (this.data.valid) this.percentage.emit(this.data.percentage);
    return this.data;
  }
  ...
  keyupAutocomplete($event:any,c:any){
