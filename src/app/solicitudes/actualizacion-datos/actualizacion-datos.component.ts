...
export class ActualizacionDatosComponent implements OnInit {
          ...
          (response: any) => {
            if (response.Status === 200) {
              ...
              if (this.solicitudForm.Documento + '' !== '0') {
                files.push({
                  Id: this.solicitudForm.Documento,
                  key: 'Documento',
                });
              }
              if (
                ...
                this.newNuxeoService.get(files).subscribe(
                  (document: any) => {
                    ...
                    ].urlTemp = document[0].url;
                    this.solicitudForm.campos[
                      this.getIndexForm('Documento')
                    ].valor = document[0].url;
                    this.loading = false;
                  },
                  () => {
                    this.loading = false;
  ...
  enviarSolicitud(event) {
    if (event.valid) {
      ...
      Swal.fire(opt).then(async (willDelete) => {
        this.loading = true;
        if (willDelete.value) {
          const files = [];
          const Solicitud: any = {};
          ...
          this.newNuxeoService.uploadFiles(files).subscribe(
            (data: any) => {
              ...
              this.solicitudDatos.FechaExpedicionActual = momentTimezone
                .tz(
                  this.solicitudDatos.FechaExpedicionActual,
                  'DD/MM/YYYY',
                  'America/Bogota'
                )
                .format('YYYY-MM-DD HH:mm:ss');
              this.solicitudDatos.FechaExpedicionActual =
                this.solicitudDatos.FechaExpedicionActual + ' +0000 +0000';
              this.solicitudDatos.FechaExpedicionNuevo = momentTimezone
                .tz(this.solicitudDatos.FechaExpedicionNuevo, 'America/Bogota')
                .format('YYYY-MM-DD HH:mm:ss');
              this.solicitudDatos.FechaExpedicionNuevo =
                this.solicitudDatos.FechaExpedicionNuevo + ' +0000 +0000';
              const hoy = new Date();
