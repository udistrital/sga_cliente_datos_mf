...
export class NewNuxeoService {
  ...
  readVerifyMimeType(file: File): Promise<boolean> {
    return new Promise<boolean>((resolve) => {
      ...
      reader.onload = () => {
        ...
        const mime = Array.from(uint8Array)
          .map((byte) => byte.toString(16).padStart(2, '0'))
          .join('')
          .toUpperCase();
        const extension = file.type || 'none';
        resolve(
          this.fileSignatures[extension].some((sign) => mime.startsWith(sign))
        );
      };
  ...
  getManyFiles(query: string) {
    ...
    this.anyService
      .getp(environment.NUXEO_SERVICE, '/document' + query)
      .subscribe(
        async (response: any) => {
          ...
          if (response.type === HttpEventType.Response) {
            ...
            let listaDocs = await Promise.all(
              listaDocsRaw.map(async (doc) => {
                if (doc.Nuxeo) {
                  return {
                    Id: doc.Id,
                    Nombre: doc.Nombre,
                    Enlace: doc.Enlace,
                    Url: await this.getUrlFile(
                      doc.Nuxeo.file,
                      doc.Nuxeo['file:content']['mime-type']
                    ),
                    TipoArchivo:
                      this.mimeTypes[doc.Nuxeo['file:content']['mime-type']],
                  };
                }
                return null;
              })
  ...
  get(files) {
    ...
    files.map((file, index) => {
      this.documentService.get('documento/' + file.Id).subscribe((doc) => {
        this.anyService
  ...
  getByUUID(uuid) {
    ...
    let documento = null;
    this.anyService
      .get(environment.NUXEO_SERVICE, '/document/' + uuid)
      .subscribe(
        async (f: any) => {
          const url = await this.getUrlFile(
            f.file,
  ...
  deleteByUUID(uuid) {
    ...
    this.anyService
      .delete2(
        environment.NUXEO_SERVICE,
        '/document/' + uuid + '?versionar=' + versionar
      )
      .subscribe(
        (r) => {
  ...
  deleteByIdDoc(Id, relacion) {
    const documentsSubject = new Subject<any>();
    const documents$ = documentsSubject.asObservable();
    this.documentService.get('documento/' + Id).subscribe(
      (doc: Documento) => {
        doc.Activo = false;
        doc.Descripcion = 'id_relacionado: ' + relacion;
        this.documentService.put('documento/', doc).subscribe(
          (doc: Documento) => {
